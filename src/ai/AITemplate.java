...
public abstract class AITemplate extends Thread {
	...
	/**
	 * Performs 1 tick's worth of common behaviour (move away from edge, move to
	 * next waypoint, etc)
	 * 
	...
	/**
	 * XXX Debug only Perform 1 tick's worth of Poirot (follow set points by A*,
	 * so hopefully isn't a lemming)
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * XXX Debug only Performs 1 tick's worth of Stubborn behaviour (brakes)
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * Performs 1 tick's worth of Roving behaviour (moves randomly)
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * Performs 1 tick's worth of Coward behaviour (runs away from all players)
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * Performs 1 tick's worth of Aggressive behaviour (runs towards all
	 * players)
	 * 
	 * @throws InterruptedException
	 */
	...
	/**
	 * Seek out the nearest player and dash once we are near them If they have
	 * moved since we planned our route, recalculate
	 * 
	 * @throws InterruptedException
	 */
	...
	private Point projectedPosition() {
		...
		int maxX = (int) resources.getMap().getWidth() - 1; // -1 in case of
															// rounding
	 ...
	 * 
	 * Move perpendicularly away from the imminent abyss TODO how would we cope
	 * if were surrounded on two/four sides? - shouldn't happen, hopefully do we
	...
	protected void moveAwayFromEdge() throws InterruptedException {
		...
		if (currentTileIndex != null) {
			...
			// n.b. These values weren't the expected ones (tile_down should be
			// row+1 by my reckoning)
		...
		if (normalToNextWaypoint != null) {
			...
			if (!normalToNextWaypoint.pointInside(curLoc)) {
				...
				if (waypoints.size() >= 2) {
					...
					lastWaypoint = waypoints.removeFirst();
					normalToNextWaypoint = normalToNextWaypoint();
					// System.out.println("Now the next waypoint is " +
					// waypoints.peek());
					return true;
				}
			}
	...
	/**
	 * Dumbly move as-the-(drunken-)crow-flies to coords. <br>
	...
	/**
	 * Given how fast we are going, for how long should we brake? This is a very
	...
	/**
	 * XXX debugging only Sets what type of AI to be
	 * 
	 * @param behaviour
	 */
	...
	/**
	 * XXX debugging only Set the waypoints to a given list
	 * 
	 * @param wp
	 */
